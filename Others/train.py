...
def get_args():
    parser = argparse.ArgumentParser(description='SAAI-Enhanced Broker-Modality Training with SSIM + Regional Loss')
    ...
    parser.add_argument('--save-dir', type=str, default='./Results1')
    
    # Original loss weights
    parser.add_argument('--alpha', type=float, default=0.1, help='Domain adversarial loss weight')
    parser.add_argument('--beta', type=float, default=0.05, help='Count consistency loss weight')
    parser.add_argument('--gamma', type=float, default=1.0, help='Density loss weight')
    
    # Enhanced loss weights for GAME improvement
    parser.add_argument('--w-ssim', type=float, default=0.5, help='SSIM structural loss weight')
    parser.add_argument('--w-regional', type=float, default=0.1, help='Regional count loss weight')
    
    # GT density map control
    parser.add_argument('--enable-enhanced-loss', action='store_true', default=False, 
                       help='Enable SSIM + Regional losses (requires GT density maps)')
...
def main():
    ...
    logger.info(f"   Enhanced Losses - SSIM: {args.w_ssim}, Regional: {args.w_regional}")
    logger.info(f"   Enhanced Mode: {'✓ Enabled' if args.enable_enhanced_loss else '✗ Disabled'}")
    
    # Setup paths
    train_path = os.path.join(args.data_path, 'train')
    val_path = os.path.join(args.data_path, 'val')
    
    # Create datasets with enhanced loss capability
    ...
    train_dataset = BrokerCrowdDataset(
        ...
        method='train',
        enable_gt_density=args.enable_enhanced_loss  # Enable GT density maps for SSIM + Regional
    )
    ...
    val_dataset = BrokerCrowdDataset(
        ...
        downsample_ratio=args.downsample_ratio, 
        method='val',
        enable_gt_density=False  # Validation doesn't need GT density maps
    )
    
    logger.info(f"📊 Dataset Info:")
    logger.info(f"   Train: {len(train_dataset)} samples")
    logger.info(f"   Val:   {len(val_dataset)} samples")
    logger.info(f"   GT Density Maps: {'✓ Enabled for training' if args.enable_enhanced_loss else '✗ Disabled'}")
    ...
    # Enhanced loss with SSIM + Regional consistency
    criterion = SAAIBrokerLoss(
        alpha=args.alpha, 
        beta=args.beta, 
        gamma=args.gamma,
        w_ssim=args.w_ssim,           # SSIM structural loss
        w_regional=args.w_regional,   # Regional count consistency
        regional_levels=(1,2,3)       # Optimize GAME(1,2,3) levels
    )
    ...
    for epoch in range(args.epochs):
        ...
        # Enhanced loss tracking with SSIM and Regional terms
        epoch_losses = {
            'total': 0, 'density': 0, 'domain': 0, 'count': 0,
            'ssim': 0, 'regional': 0,  # Enhanced loss tracking
        ...
        epoch_pbar = tqdm(train_loader, desc=f"Epoch {epoch+1:3d}/{args.epochs}", 
                         ncols=150, leave=False)  # Wider for enhanced info
        
        for batch_idx, batch_data in enumerate(epoch_pbar):
            # Handle enhanced training with GT density maps for SSIM + Regional losses
            if len(batch_data) == 6:  # Enhanced training with GT density
                rgb, thermal, keypoints_list, targets_list, st_sizes, gt_density = batch_data
                rgb, thermal = rgb.to(device), thermal.to(device)
                gt_density = gt_density.to(device)  # Move GT density to GPU
                
                # Log enhanced training activation (first epoch, first batch only)
                if batch_idx == 0 and epoch == 0:
                    logger.info("🔬 Enhanced training ACTIVE: SSIM + Regional losses enabled")
                    logger.info(f"   GT density shape: {gt_density.shape}")
                    logger.info(f"   GT density range: [{gt_density.min():.4f}, {gt_density.max():.4f}]")
                    logger.info(f"   GT density mean: {gt_density.mean():.4f}")
                    
            else:  # Original training without GT density
                rgb, thermal, keypoints_list, targets_list, st_sizes = batch_data
                rgb, thermal = rgb.to(device), thermal.to(device)
                gt_density = None
                
                # Log original training mode (first epoch, first batch only)
                if batch_idx == 0 and epoch == 0:
                    logger.info("📊 Standard training mode: Bayesian + Domain + Count losses only")
            ...
            # Enhanced loss calculation - SSIM + Regional activate when gt_density provided
            total_loss, loss_dict = criterion(
                density_map, keypoints_list, targets_list,
                domain_pred_rgb, domain_pred_thermal,
                gt_density_maps=gt_density  # Enables SSIM + Regional when not None
            )
            ...
            epoch_losses['ssim'] += loss_dict['ssim_loss']        # Should be > 0 if enhanced mode
            epoch_losses['regional'] += loss_dict['regional_loss'] # Should be > 0 if enhanced mode
            epoch_losses['train_mae'] += batch_mae
            epoch_losses['train_rmse'] += batch_rmse
            batch_count += 1
            
            # Enhanced progress bar with meaningful loss info
            progress_info = {
                'Loss': f'{total_loss.item():.4f}',
                'Den': f'{loss_dict["density_loss"]:.3f}',
                'Dom': f'{loss_dict["domain_loss"]:.3f}',
                'MAE': f'{batch_mae:.1f}',
            }
            
            # Add enhanced loss info when active
            if gt_density is not None:
                progress_info['SSIM'] = f'{loss_dict["ssim_loss"]:.3f}'
                progress_info['Reg'] = f'{loss_dict["regional_loss"]:.3f}'
        ...
        # Enhanced epoch logging with detailed loss breakdown
        ...
        # Enhanced loss logging with activation status
        if avg_losses['ssim'] > 0 or avg_losses['regional'] > 0:
            logger.info(f"      SSIM:    {avg_losses['ssim']:.6f} {'✓ Active' if avg_losses['ssim'] > 0 else '✗ Inactive'}")
            logger.info(f"      Regional:{avg_losses['regional']:.6f} {'✓ Active' if avg_losses['regional'] > 0 else '✗ Inactive'}")
        else:
            logger.info(f"      SSIM:    {avg_losses['ssim']:.6f} ✗ No GT density maps")
            logger.info(f"      Regional:{avg_losses['regional']:.6f} ✗ No GT density maps")
        ...
        if (epoch + 1) % 10 == 0:
            ...
            for metric_name, current_value in val_metrics.items():
                if current_value < best_metrics[metric_name]:
                    ...
                    torch.save({
                        ...
                        'enhanced_loss_weights': {
                            'w_ssim': args.w_ssim,
                            'w_regional': args.w_regional,
                            'enhanced_mode': args.enable_enhanced_loss
                        },
                        'args': vars(args)
                    }, save_path)
            ...
            if not improved_metrics:
                ...
                logger.info(f"🎯 Improvements in: {', '.join(improved_metrics)}")
            
            # Display best metrics summary (Broker-Modality format)
            logger.info("🏆 Best Metrics So Far (Enhanced Broker-Modality Format):")
    ...
    # Training completion summary (Enhanced Broker-Modality format)
    logger.info("=" * 80)
    logger.info("🎉 Enhanced Training Completed!")
    logger.info("🏆 Final Best Metrics (Enhanced Broker-Modality Format):")
    logger.info(f"   GAME(0): {best_metrics['GAME_0']:.2f} (Global counting accuracy)")
    logger.info(f"   GAME(1): {best_metrics['GAME_1']:.2f} (2×2 grid accuracy)")
    logger.info(f"   GAME(2): {best_metrics['GAME_2']:.2f} (4×4 grid accuracy)")  
    logger.info(f"   GAME(3): {best_metrics['GAME_3']:.2f} (8×8 grid accuracy)")
    logger.info(f"   RMSE:    {best_metrics['RMSE']:.2f} (Root mean square error)")
    
    logger.info(f"💾 All best models saved in: {args.save_dir}")
    logger.info(f"📝 Complete log saved: {log_file}")
    
    # Save enhanced final results with proper float conversion
    results = {
        'method': 'SAAI-Enhanced Broker-Modality with SSIM + Regional Loss',
        'venue': 'Enhanced Custom Implementation',
        'dataset': 'RGBT-CC',
        'enhancement_details': {
            'ssim_weight': float(args.w_ssim),
            'regional_weight': float(args.w_regional),
            'enhanced_mode_enabled': bool(args.enable_enhanced_loss),
            'target_improvement': 'GAME(1,2,3) spatial accuracy while maintaining GAME(0)',
        },
        'final_metrics': {
            'GAME_0': float(best_metrics['GAME_0']),
            'GAME_1': float(best_metrics['GAME_1']),
            'GAME_2': float(best_metrics['GAME_2']),
            'GAME_3': float(best_metrics['GAME_3']),
            'RMSE': float(best_metrics['RMSE'])
        },
        'comparison_format': 'Compatible with ECCV 2024 Broker-Modality paper',
        'enhancement_status': 'SSIM + Regional losses for better spatial localization' if args.enable_enhanced_loss else 'Standard SAAI training'
    }
    
    try:
        with open(os.path.join(args.save_dir, 'enhanced_broker_modality_results.json'), 'w') as f:
            json.dump(results, f, indent=2)
        logger.info("💾 Results successfully saved to JSON")
    except Exception as e:
        logger.error(f"❌ Error saving results: {e}")
    
    # Final enhancement summary
    logger.info("🔬 Enhancement Summary:")
    if args.enable_enhanced_loss:
        logger.info("   ✅ SSIM Loss: Enforced structural similarity between pred and GT density maps")
        logger.info("   ✅ Regional Loss: Directly optimized grid-level count accuracy (GAME metrics)")
        logger.info("   🎯 Expected: Lower GAME(1,2,3) with maintained GAME(0) performance")
    else:
        logger.info("   📊 Standard SAAI training: Bayesian + Domain + Count losses")
        logger.info("   💡 To enable enhanced losses, use --enable-enhanced-loss flag")
